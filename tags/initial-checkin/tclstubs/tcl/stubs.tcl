#
# $Header: /home/rhseeger/svn/tclstubs-cvsbackup/tclstubs/tcl/stubs.tcl,v 1.1.1.1 2004-07-20 17:22:56 robert_seeger Exp $
#
#     The stubs package
#
...
namespace eval ::stubs {
    ...
    proc moveProc {name} {
        ...
        if {[llength [uplevel 1 [list info command $name]]] } {
            set stubName "[namespace current]::stubbedProcs::stubbed-$name-[incr stubCount]"
            uplevel 1 [list ::stubs::rename $name $stubName]
        } else {
            set stubName {}
        }

        if { [info exists stubProcs($name)] && [string length $stubName]} {
            # Stubbing an already stubbed proc
            set stubStubs($stubName) [lindex $stubProcs($name) end]
        }

        lappend stubProcs($name) $stubName
        return $stubName
    }
